...
//    touch_dot.y = (((touch_dot.y_ad_val * 320)>>12)-150)*2;
	...
	touch_dot.x=((touch_dot.x_ad_val-90)/5.500); // 把读到的X_ADC值转换成TFT X坐标值
	touch_dot.y=((touch_dot.y_ad_val-115)/4.205); // 把读到的Y_ADC值转换成TFT Y坐标值//    touch_dot.x = (((touch_dot.x_ad_val * 240)>>12)-110)*2;
